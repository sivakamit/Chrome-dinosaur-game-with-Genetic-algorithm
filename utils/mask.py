    ...
    for i in range(0, len(new_image)):
        ...
        for j in range(reduceRatio):
            ...
            for k in range(0, reduceRatio):
                original_pixel_pos = (line_count*originalDimension[0]*(reduceRatio-1))+(
                    i*reduceRatio)+j + ((k)*originalDimension[0])
    ...
    for i in range(img_length-1):
        for j in range(img_length-1):
            if(image_vector[j]['x'] > image_vector[j+1]['x']):
                image_vector[j], image_vector[j +
                                              1] = image_vector[j+1], image_vector[j]
...
def generateMaskData(imageName, maskName):
    ...
    image_border = getBorder(
        image_resized['image'], image_resized['dimensions'])
    image_border_coords = getBorderCoords(
        image_border['image'], image_border['dimensions'])
