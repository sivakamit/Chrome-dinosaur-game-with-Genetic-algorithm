...
class FlyingDino:
    def __init__(self, master, canvas, onCollidChange, onScreenOut=lambda x=None: x, height=70):
        self.canvas = canvas
        self.master = master
        self.moving_id = None
        self.onScreen = False
        self.onScreenOut = onScreenOut
        self.height = abs(height-150)
        ...
        self.id = canvas.create_image(
            900, self.obj_height, image=self.image, anchor=NW)
        # load mask
