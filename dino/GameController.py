...
from dino.CollisionMonitor import ColisionMonitor
from dino.Dino import Dino
from dino.Cactus import Cactus
from dino.FlyingDino import FlyingDino
from dino.CollisionMonitor import ColisionMonitor
from dino.ObstacleGenerator import ObstacleGenerator
from dino.DinoBrain import DinoBrain
...
class GameController:
    def __init__(self, mode, dinos_per_gen=10):
        ...
        self.colisionMonitor = ColisionMonitor(
            self.master, self.canvas, self.stopGround)
        self.dinos = []
        ...
        self.game_params = {'distance': 100,
                            'speed': 25, 'height': 1, 'width': 50}
        self.master.bind('<r>', self.restart)
        self.scoresCheckPoint = [5, 10, 15, 20, 25,
                                 30, 35, 40, 45, 50, 80, 110, 140, 200, 250, 290, 320, 340, 350, 400]
        ...
        self.ground_id = self.canvas.create_image(
            0, 195, image=self.ground, anchor=NW)
        self.ground_id_1 = self.canvas.create_image(
            400, 195, image=self.ground_1, anchor=NW)
        self.ground_id_2 = self.canvas.create_image(
            800, 195, image=self.ground, anchor=NW)
    ...
    def prepareInterface(self):
        ...
        speedLabel = Label(self.infoPanel, text="Speed: "+str(
            self.game_params['speed'])+"/"+str(len(self.scoresCheckPoint)), bg='#fff', font=("Arial", 20))
        speedLabel.grid(row=0, column=0, pady=10, sticky=W)
        self.interfaceObject['speedLabel'] = speedLabel

        dinosAlive = Label(self.infoPanel, text="Dinos: " +
                           str(self.initialDinoNum), bg='#fff', font=("Arial", 20))
        dinosAlive.grid(row=1, column=0, pady=10, sticky=W)
        self.interfaceObject['dinosAlive'] = dinosAlive

        scoreLabel = Label(self.infoPanel, text="Score: " +
                           str(self.score), bg='#fff', font=("Arial", 20))
        scoreLabel.grid(row=2, column=0, pady=10, sticky=W)
        self.interfaceObject['score'] = scoreLabel

        record = Label(self.infoPanel, text="Record: " +
                       str(self.record), bg='#fff', font=("Arial", 20))
        record.grid(row=0, column=1, padx=20, pady=10, sticky=W)
        self.interfaceObject['record'] = record

        n_generation = Label(
            self.infoPanel, text="Generation: "+str(self.n_generations), bg='#fff', font=("Arial", 20))
        n_generation.grid(row=1, column=1, padx=20, pady=10, sticky=W)
        self.interfaceObject['n_generation'] = n_generation

        general_record = Label(
            self.infoPanel, text="General record: "+str(self.general_record), bg='#fff', font=("Arial", 20))
        ...
        #self.nn_elements = draw_nn(width=self.width, height=self.height, nn_shape=[5, 2], weights=weights, weights_flatten=weights_flatten, biases=biases,canvas=self.canvas, padding=[50, 10, 400, 300], neuron_size=10)
    ...
    def animateGround(self):
        ...
        if(self.canvas.coords(self.ground_id)[0] < 0):
            self.canvas.move(self.ground_id, 1200, 0)
        if(self.canvas.coords(self.ground_id_1)[0] < 0):
            self.canvas.move(self.ground_id_1, 1200, 0)
        if(self.canvas.coords(self.ground_id_2)[0] < 0):
    ...
    def updateLabels(self):
        self.interfaceObject['speedLabel'].config(
            text="Speed: "+str(25 - self.game_params['speed'])+"/"+str(len(self.scoresCheckPoint)))
        self.interfaceObject['dinosAlive'].config(
            text="Dinos: "+str(self.dinosOnScreen)+"/"+str(self.initialDinoNum))
        ...
        self.interfaceObject['n_generation'].config(
            text="Generation: "+str(self.n_generations))
        self.interfaceObject['general_record'].config(
            text="General record: "+str(self.general_record))
    # create game elements

    def prepareGame(self):
        self.dinos.append(Dino(self.master, self.canvas, DinoBrain(), self.game_params,
                          self.decreaseDinos, mode=self.mode, game_modes=self.game_modes, color="black"))
        self.dinos.append(Dino(self.master, self.canvas, DinoBrain(), self.game_params,
                          self.decreaseDinos, mode="train", game_modes=self.game_modes, color="red"))
        # self.dinos.append(Dino(self.master, self.canvas, DinoBrain(), self.game_params,
        #                   self.decreaseDinos, mode="simulation", game_modes=self.game_modes, color="blue"))
        self.dinos[-1].brain.load()
        self.dinosOnScreen = 2
        self.obstacleGenerator = ObstacleGenerator(
            self.master, self.canvas, self.updateGameParams, self.increaseScore)
        self.obstacleGenerator.updateObstaclesSpeed(self.game_params['speed'])
        self.obstacleGenerator.run()
        self.colisionMonitor = ColisionMonitor(
            self.master, self.canvas, self.stopGround, self.dinos, self.obstacleGenerator.obstacles, self.dinosOnScreen)
    ...
    def prepareTrain(self):
        for i in range(self.initialDinoNum):
            self.dinosOnScreen += 1
            dino = Dino(self.master, self.canvas, DinoBrain(), self.game_params,
                        self.decreaseDinos, mode=self.mode, game_modes=self.game_modes)
            dino.brain.load()
    ...
    def increaseScore(self, score):
        ...
        if(self.score in self.scoresCheckPoint):
            ...
            self.obstacleGenerator.updateObstaclesSpeed(
