...
class ObstacleGenerator:
    def __init__(self, master, canvas, updateGameParams, increaseScore):
        ...
        self.master = master
        self.obstacles = [
            Cactus(self.master, self.canvas, self.updateFirstObstacle,
                   3, self.decraseObstaclesOnscreen),
            Cactus(self.master, self.canvas, self.updateFirstObstacle,
                   1, self.decraseObstaclesOnscreen),
            Cactus(self.master, self.canvas, self.updateFirstObstacle,
                   2, self.decraseObstaclesOnscreen),
            FlyingDino(self.master, self.canvas,
                       self.updateFirstObstacle, self.decraseObstaclesOnscreen),
            FlyingDino(self.master, self.canvas, self.updateFirstObstacle, self.decraseObstaclesOnscreen, 125)]
    ...
    def run(self):
        if(len(self.obstaclesIndexQueue) > 0):
            ...
            self.updateGameParams(distance=obstacle_distance,
                                  height=obstacle.height, width=obstacle.width)
    ...
    def spawnObstacle(self):
        ...
        for obstacle in self.obstacles:
            ...
            count += 1
        self.obstaclesOnScreen += 1
        obstacle_index = avaliable_index[random.randint(
            0, len(avaliable_index)-1)]
