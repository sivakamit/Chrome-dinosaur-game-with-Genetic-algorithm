...
from data.mask.generateMaskData import generateMaskData
import pickle
import numpy as np
import random
...
class Dino:
    def __init__(self, master, canvas, brain, game_params, decreaseDinos, jump_height=100, mode="game", game_modes={}, color=None):
        ...
        self.best = False
        if(color is None):
            self.color = ['black', 'green', 'blue', 'red',
                          'yellow', 'orange'][random.randint(0, 5)]
        else:
            self.color = color
        # load default image
        ...
        self.id = self.canvas.create_image(
            100, 150, image=self.image, anchor=NW)
        # load mask
    ...
    def animate(self):
        if(not self.moving):
            if(not self.bent):
                self.animation_raise_id = self.canvas.after(
                    130, self.changeRaiseImage)
            else:
                self.moving_bent_id = self.canvas.after(
                    200, self.changeBentImage)
    ...
    def changeRaiseImage(self):
        if(not self.bent):
            ...
            self.image = ImageTk.PhotoImage(
                self.imgs_pil_running[self.current_pil_running_index])
    ...
    def changeBentImage(self):
        if(self.bent):
            ...
            if(self.current_pil_bent_running_index > 1):
                self.current_pil_bent_running_index = 0
            self.image = ImageTk.PhotoImage(
                self.imgs_pil_bent_running[self.current_pil_bent_running_index])
            self.canvas.itemconfig(self.id, image=self.image)
        self.animate()
    ...
    def jump(self, event):
        ...
        elif(self.distance >= self.jump_height and self.canvas.coords(self.id)[-1] < 150):
            self.distance += 1
            self.move(0, 1)
            self.moving_id = self.canvas.after(3, self.jump, event)
        else:
